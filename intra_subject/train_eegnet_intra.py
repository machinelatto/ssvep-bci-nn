...
def run_and_evaluate_subject(
    ...
    all_recall = []
    all_f1 = []
    subj_folder = RESULTS_DIR.joinpath(f"{test_subject + 1}")
    ...
    for trial in range(0, 6):
        ...
        model = EEGNetv4(n_chans=9, n_outputs=n_classes, n_times=250).to(device)
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.0001)

        # Train the model
        best_model_save_path = subj_folder.joinpath(
            f"{test_subject + 1}_best_model_trial_{trial}.pth"
        )
        ...
        # Eval
        best_model = EEGNetv4(n_chans=9, n_outputs=n_classes, n_times=250)
        best_model.load_state_dict(torch.load(best_model_save_path))
        best_model.to(device)
        cm_path = subj_folder.joinpath(f"{test_subject + 1}_cm_trial_{trial}.png")
...
if __name__ == "__main__":
    RESULTS_DIR = Path("EEGNET_INTRA")
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # DATASET = Path("../processed_datasets/sinais_filtrados_6_70_Hz_janelas_1s/")
    DATASET = Path("processed_40_classesTrue.npy")
    LABELS = Path("labels_processed_40_classesTrue.npy")
    ...
    results_dict = run_model_for_all_subjects(
        DATASET, LABELS, 64, channels, selected_freqs, epochs
    )
