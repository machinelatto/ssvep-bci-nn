"""
Run DESTINE EEGNet+CCA cross-subject experiments via CLI.

Example:
python run_destine_eegnetcca_experiments.py \
    --users subject_01,subject_02,subject_03 \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DESTINE EEGNet+CCA cross-subject experiments")
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument("--weight-decay", type=float, default=7.5e-5)
    ...
    parser.add_argument(
        ...
        help="Comma-separated channel indices used for EEG input.",
    )
    parser.add_argument(
        "--results-dir",
        type=str,
        default="destine_results/eegnetcca",
    )
...
def main():
    ...
    try:
        ...
        raise RuntimeError(
            "Could not import braindecode EEGNet. Verify braindecode/wfdb/pandas compatibility in the active environment."
        ) from exc
    ...
    print(f"CCA mode: {args.cca_mode}")
    ...
    for test_user in test_users:
        ...
        if args.cca_mode == "joint":
            x_train, x_test, labels_train, labels_test, channels_for_model = build_tensors_with_cca_joint(
            ...
            x_train, x_test, labels_train, labels_test, channels_for_model = build_tensors_with_cca(
                train_data=train_data,
                test_data=test_data,
                occipital_electrodes=occipital_electrodes,
                frequencias=freq_arr,
        ...
        model = EEGNet(
            ...
            n_outputs=len(frequencies),
            n_times=window_samples,
            kernel_length=(args.sample_rate // 2),
            F1=8,
            D=2,
            drop_prob=0.25,
        ).to(device)
    ...
    print("\nAll DESTINE EEGNet+CCA experiments completed.")
