"""
Run DESTINE EEGNet+FBCCA cross-subject experiments via CLI.
"""
...
class EEGNetWithSubbandMerge(nn.Module):
    ...
    def __init__(self, subbands, n_chans, n_outputs, n_times, kernel_length, F1=8, D=2, drop_prob=0.3):
        ...
        self.eegnet = self._build_eegnet(
            n_chans=n_chans,
            n_outputs=n_outputs,
            n_times=n_times,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DESTINE EEGNet+FBCCA cross-subject experiments")
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument(
        "--results-dir",
        type=str,
        default="destine_results/eegnetfbcca",
    )
    parser.add_argument(
        "--fbcca-mode",
        type=str,
...
def main():
    ...
    print("\nAll DESTINE EEGNet+FBCCA experiments completed.")
