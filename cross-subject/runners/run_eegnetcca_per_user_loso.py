"""
Run EEGNet with CCA tensors using leave-one-session-out per user.
"""
...
for tamanho_da_janela_seg_val in tamanho_da_janela_seg:
    ...
    exp_root = Path(
        "/home/mateuschinelatto/Experiments/ssvep-bci-nn/cross-subject/louo_experiments/models/eegnetcca_per_user_loso"
    )
    ...
    for user_id in users_to_run:
        ...
        for session_idx in range(num_sessions):
            ...
            X_train = torch.from_numpy(tensor_treinamento.copy()).float().to(device)
            Y_train = labels_train.to(torch.long).to(device)
            X_test = torch.from_numpy(tensor_teste.copy()).float().to(device)
            ...
            test_loader = DataLoader(test_dataset, batch_size=116, shuffle=False)
