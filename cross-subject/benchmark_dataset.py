...
def build_tensors_with_cca_joint(
    ...
    x_train_cca = np.zeros(
        (len(occipital_electrodes), total_time_train)
    )
    ...
    # CCA calculated jointly per frequency across all trials simultaneously
    combinadores_x = []

    for k in range(len(indices)):
        # Build only the current frequency reference to avoid a very large 3D allocation.
        y_ref = reference_matrix(
            num_harmonica,
            inform_fase,
...
def build_tensors_with_fbcca(
    ...
    # Filter into subbands (returns: trials, subbands, channels, time)
    x_train_windows_sub = filter_signals_subbands(x_train_windows_raw, subban_no=subban_no, sampling_rate=sampling_rate)
    x_test_windows_sub = filter_signals_subbands(x_test_windows_raw, subban_no=subban_no, sampling_rate=sampling_rate)
...
def build_tensors_with_fbcca_joint(
    ...
    # Build one joint training matrix per subband by concatenating all frequencies/trials.
    for sub in range(subban_no):
...
def build_tensors_ttcca(
    ...
    """Build CCA-projected train/test tensors and labels using treanferred template CCA."""
